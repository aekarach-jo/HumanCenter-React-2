...
const BlackList = () => {
  ...
  const title = 'Black List';
  const description = '';

  const [isDeleting, setIsDeleting] = useState(false);
  const [filter, setFilter] = useState({ filters: 'blacklist:eq:1' });
  ...
  const columns = useMemo(() => {
    return [
      ...
      },
      {
        ...
        sortable: false,
        headerClassName: 'text-medium text-muted-re',
        Cell: ({ cell }) => <div className="text-medium">{moment.utc(cell?.value || new Date()).format('DD/MM/YYYY') || '-'}</div>,
      },
      {
        Header: 'จัดการข้อมูล',
        accessor: 'action',
